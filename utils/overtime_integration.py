...
OVERTIME_NETWORK_ID = int(os.getenv("OVERTIME_NETWORK_ID", "10"))  # 10=Optimism (default)
...
def _overtime_get_markets(network_id: int) -> list[dict]:
    ...
    r.raise_for_status()
    data = r.json()
    return [m for m in (data if isinstance(data, list) else []) if m.get("sport") == OVERTIME_SPORT]

def _odds_map(m: dict) -> dict:
    arr = m.get("odds") or []
    if len(arr) >= 3:
        return {"home": float(arr[0]["decimal"]), "draw": float(arr[1]["decimal"]), "away": float(arr[2]["decimal"])}
    if len(arr) == 2:
        return {"home": float(arr[0]["decimal"]), "away": float(arr[1]["decimal"])}
    out = {}
    for i, o in enumerate(arr):
        out[f"pos_{i}"] = float(o.get("decimal", 0))
    return out
...
def _sb_upsert_row(row: Dict[str, Any]) -> Tuple[int, str]:
    ...
    # 42P10: no unique index → manual upsert (DELETE+INSERT)
    msg = (r.text or "").lower()
...
# ── public API ────────────────────────────────────────────────────────────────
def build_matches_ot_row(ot_market: dict, fixture_id: Optional[int] = None) -> Dict[str, Any]:
...
def upsert_overtime_from_fixture(match_json: dict) -> Tuple[int, str]:
    ...
    1) fetch Overtime markets
    2) find best match by team names & kickoff
    3) upsert to public.matches_ot
    ...
    # enrich with networkId for row builder
    best = dict(best)
    best["networkId"] = best.get("networkId") or OVERTIME_NETWORK_ID

    row = build_matches_ot_row(best, fixture_id=fixture_id)
