...
PATCH_VP_RESULTS = os.getenv("PATCH_VP_RESULTS", "false").lower() in ("1","true","yes","y")
...
def _fetch_fixtures_for_date(date_str: str) -> List[Dict[str, Any]]:
    data = _http_get(f"{BASE_URL}/fixtures", params={"date": date_str}) or {}
...
def _upsert_verification(rows: List[Dict[str, Any]]) -> bool:
    try:
        supabase.table("verifications").upsert(rows, on_conflict="prediction_id").execute()
        return True
    except Exception as e:
...
def _try_patch_value_predictions_result(pid: str, patch: Dict[str, Any]) -> None:
    if not PATCH_VP_RESULTS:
        return  # disabled by default to avoid PGRST204 noise
    try:
        supabase.table("value_predictions").update(patch).eq("id", pid).execute()
    except Exception as e:
        msg = str(e)
        # Keep this INFO level & short; it often means columns not present.
...
def _ou25_is_correct(pick: str, total_goals: int) -> Optional[bool]:
    ...
    if p == "over":  return total_goals >= 3
    if p == "under": return total_goals <= 2
    return None
