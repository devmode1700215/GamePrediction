...
def safe_extract_match_data(match):
    ...
    try:
        ...
        if not fixture or not teams or not league:
            logger.error(f"❌ Invalid match structure for fixture")
            return None
        ...
        if not home_team or not away_team:
            logger.error(f"❌ Missing team data for fixture")
            return None
        ...
        if not fixture_id or not date:
            logger.error(f"❌ Missing fixture ID or date")
            return None
...
def main():
    try:
        ...
        # Fetch today and tomorrow to cover 24h range
        try:
            ...
            # check fixtures is array
            if not isinstance(fixtures, list):
                logger.error(f"❌ Fixtures is not an array")
                return
        except Exception as e:
        ...
        # Filter only matches within the next 24h
        try:
            fixtures = [
                f for f in fixtures
        ...
        for match in fixtures:
            try:
                ...
                # Get additional team data with error handling
                ...
                # Fetch recent goals
                home_goals = get_recent_goals(home_team['id'])
                away_goals = get_recent_goals(away_team['id'])
                home_team['recent_goals'] = home_goals
                away_team['recent_goals'] = away_goals

                # Fetch injuries
                home_team['injuries'] = get_team_injuries(home_team['id'], season)
                away_team['injuries'] = get_team_injuries(away_team['id'], season)

                # Fetch odds and head-to-head
                ...
                try:
                    ...
                    # Insert value predictions
                    insert_value_predictions(prediction)
                    successful_predictions += 1
                    logger.info(f"✅ Processed prediction for fixture {fixture_id}")
                    
                except Exception as e:
                    logger.error(f"❌ Error processing prediction for fixture {fixture_id}: {e}")
                    failed_predictions += 1
                    continue

            except Exception as e:
                logger.error(f"❌ Unexpected error processing match: {e}")
                failed_predictions += 1
                continue
