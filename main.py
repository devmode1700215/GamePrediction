...
# enrich_fixture may or may not exist depending on the branch; make it optional.
try:
    ...
    def enrich_fixture_safe(fx_raw: Dict[str, Any], preferred_bookmaker: str = PREFERRED_BOOK) -> Dict[str, Any]:
        try:
            ...
            logging.warning(f"enrich_fixture failed for fixture {(fx_raw.get('fixture') or {}).get('id')}: {e}")
            return fx_raw
    _HAS_ENRICH = True
except Exception:
    logging.info("utils.get_football_data.enrich_fixture not found; proceeding without enrichment.")
    def enrich_fixture_safe(fx_raw: Dict[str, Any], preferred_bookmaker: str = PREFERRED_BOOK) -> Dict[str, Any]:
        # Minimal passthrough; still attach odds to keep pipeline alive
        fid = (fx_raw.get("fixture") or {}).get("id")
        try:
            odds = get_match_odds(fid) if fid else {}
        except Exception:
            odds = {}
        out = dict(fx_raw)
        out["odds"] = odds
...
from utils.get_prediction import get_prediction  # <- your restored weighted engine lives here now
...
# ----------------------------
# Build prediction payload (matches your previous shape)
# ----------------------------
def _build_llm_payload_from_enriched(
    ...
    payload = {
        ...
        # Richer blocks for the (now internal) weighted engine in get_prediction.py
        "recent_form": {
...
def _process_fixture(fx_raw: Dict[str, Any]) -> None:
    """
    Enrich -> insert match -> predict (classic weighted in get_prediction) -> (optionally invert) -> store prediction.
    """
    ...
    # 1) Enrich the fixture if available (form, season context, injuries, lineups, OU/BTTS)
    ...
    # 4) Model prediction (classic weighted engine implemented inside utils/get_prediction.py)
    try:
...
def _process_fixtures_for_date(date_str: str) -> None:
    ...
    try:
        ...
        # Keep a quick info line here as a health signal
...
if __name__ == "__main__":
    ...
    if argv_ids:
        ...
        # Default: process today only (APP_TZ; default UTC)
        today_str = _today_str_app()
        _process_fixtures_for_date(today_str)

    # --- Result settlement (today & yesterday, APP_TZ) ---
    if _HAS_SETTLER:
        try:
