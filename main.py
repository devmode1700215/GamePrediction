...
from utils.odds_sources import get_ou25_best  # NEW
...
def _build_llm_payload_from_enriched(fx_enriched: Dict[str, Any], odds_block: Dict[str, Any], odds_src: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "head_to_head": h2h,
        "odds": {"over_2_5": odds_block.get("over_2_5"), "under_2_5": odds_block.get("under_2_5"), "source": odds_src},
...
def _normalize_prediction(pred: Dict[str, Any], payload_odds: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try: p_over = float(p_over) if p_over is not None else None
    except Exception: p_over = None
    ...
    try: chosen_odds = float(chosen_odds) if chosen_odds is not None else None
    except Exception: chosen_odds = None
...
def _process_fixture(fx_raw: Dict[str, Any]) -> None:
    ...
    # 2) Get best OU2.5 odds (Overtime first, then API-Football)
    ...
    # 4) Optional inversion (kept off by default)
    ...
    try:
        ...
        if count:
            logging.info(
                ...
                f"| conf={final_pred.get('confidence')} | src={odds_block.get('source')} | {msg}"
            )
        else:
...
def _fetch_single_fixture_stub(fixture_id: int) -> Dict[str, Any]:
    # In CLI mode we still want odds from the adapter
    return {"fixture": {"id": fixture_id}, "teams": {}, "league": {}, "odds": {}}
